...
export default function EventRowActions({
  ...
  return (
    <div className="flex items-center justify-end gap-1">
      ...
      <EditEvent row={row} clients={clients} open={editOpen} onClose={() => setEditOpen(false)} />
      <ConfirmDialog
        open={confirmOpen}
        onClose={() => setConfirmOpen(false)}
        onConfirm={() => deleteEventAction(row.event_id)}
        title="Delete event?"
        description={`Permanently deletes "${row.name}" and its entire guest list.`}
