...
import { EVENT_GUEST_IMPORT_FIELD_HELP } from '@/lib/event-import';
import { importGuestsAction } from '../actions';

export default function ImportGuests({ eventId }: { eventId: string }) {
  const [open, setOpen] = useState(false);
  const [state, action] = useActionState(importGuestsAction, IMPORT_INITIAL);
  ...
  return (
    <>
      <button
        ...
        className="inline-flex w-full items-center justify-center gap-2 rounded-md border border-aegis-gray-200 bg-white px-4 py-2 text-sm font-medium text-aegis-navy shadow-sm transition-colors hover:bg-aegis-gray-50 sm:w-auto"
      >
        <svg
          className="h-4 w-4"
          viewBox="0 0 24 24"
      ...
      <Modal
        ...
        description="Upload a CSV file. Re-imports skip rows that already exist for this event."
      >
        <form action={action} className="space-y-5">
          ...
          <section className="rounded-md border border-aegis-gray-100 bg-aegis-gray-50 px-4 py-3">
            ...
            <p className="mt-1 text-xs text-aegis-gray-500">
              Open it in Excel, fill in your guests, then save as CSV (UTF-8).
            </p>
            <a
          ...
          <section>
            ...
            <ul className="mt-2 space-y-1.5 text-xs text-aegis-gray">
              {EVENT_GUEST_IMPORT_FIELD_HELP.map((f) => (
          ...
          {state.ok && state.imported > 0 && (
            <div className="rounded-md border border-green-300 bg-green-50 px-3 py-2 text-xs text-green-800">
              Imported {state.imported} guest{state.imported === 1 ? '' : 's'} successfully.
              {state.duplicates > 0 && (
          ...
          {state.ok && state.imported === 0 && state.duplicates > 0 && (
            <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-800">
              All {state.duplicates} row{state.duplicates === 1 ? '' : 's'} matched existing guest{state.duplicates === 1 ? '' : 's'} and were skipped.
            </div>
          )}
          ...
          {state.errors.length > 0 && (
            <div className="rounded-md border border-aegis-orange/30 bg-white">
              ...
              <ul className="aegis-scroll max-h-40 divide-y divide-aegis-gray-100 overflow-y-auto text-xs text-aegis-gray">
                {state.errors.map((e) => (
