...
import EditMediaContact from './edit-media-contact';
import { deleteMediaContactAction } from './actions';
import type { MediaContact } from '@/lib/types';

export default function MediaRowActions({ row }: { row: MediaContact }) {
  ...
  return (
    <div className="flex items-center justify-end gap-1">
      ...
      <EditMediaContact row={row} open={editOpen} onClose={() => setEditOpen(false)} />
      <ConfirmDialog
        open={confirmOpen}
        onClose={() => setConfirmOpen(false)}
        onConfirm={() => deleteMediaContactAction(row.media_id)}
        title="Delete media contact?"
        description={`This will permanently delete "${row.full_name}".`}
