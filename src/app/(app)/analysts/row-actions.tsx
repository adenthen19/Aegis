...
import EditAnalyst from './edit-analyst';
import { deleteAnalystAction } from './actions';
import type { Analyst } from '@/lib/types';

export default function AnalystRowActions({ row }: { row: Analyst }) {
  ...
  return (
    <div className="flex items-center justify-end gap-1">
      ...
      <EditAnalyst row={row} open={editOpen} onClose={() => setEditOpen(false)} />
      <ConfirmDialog
        open={confirmOpen}
        onClose={() => setConfirmOpen(false)}
        onConfirm={() => deleteAnalystAction(row.investor_id)}
        title="Delete analyst?"
        description={`This will permanently delete "${row.institution_name}".`}
