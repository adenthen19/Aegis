...
import { ANALYST_IMPORT_FIELD_HELP } from '@/lib/analyst-import';
import { IMPORT_INITIAL } from '@/lib/csv';
import { importAnalystsAction } from './actions';

export default function ImportAnalysts() {
  const [open, setOpen] = useState(false);
  const [state, action] = useActionState(importAnalystsAction, IMPORT_INITIAL);
  ...
  return (
    <>
      ...
      <Modal
        ...
        title="Import analysts from Excel"
        ...
        <form action={action} className="space-y-5">
          ...
          <section>
            ...
            <ul className="mt-2 space-y-1.5 text-xs text-aegis-gray">
              {ANALYST_IMPORT_FIELD_HELP.map((f) => (
          ...
          {state.ok && state.imported > 0 && (
            <div className="rounded-md border border-green-300 bg-green-50 px-3 py-2 text-xs text-green-800">
              Imported {state.imported} analyst{state.imported === 1 ? '' : 's'} successfully.
              {state.duplicates > 0 && (
          ...
          {state.ok && state.imported === 0 && state.duplicates > 0 && (
            <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-800">
              All {state.duplicates} row{state.duplicates === 1 ? '' : 's'} matched existing analyst{state.duplicates === 1 ? '' : 's'} and were skipped.
            </div>
          )}
