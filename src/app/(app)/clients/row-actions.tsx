...
export default function ClientRowActions({ row }: { row: Client }) {
  ...
  return (
    <div className="flex items-center justify-end gap-1">
      ...
      <EditClient row={row} open={editOpen} onClose={() => setEditOpen(false)} />
      <ConfirmDialog
        open={confirmOpen}
        onClose={() => setConfirmOpen(false)}
        onConfirm={() => deleteClientAction(row.client_id)}
        title="Delete client?"
        description={`This will permanently delete "${row.corporate_name}" and any related projects/meetings will lose the client link.`}
